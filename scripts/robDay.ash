...
void consume1()
{
	...
	getChew(4,$item[grim fairy tale]);
...
void freeRun(location place)
{
	...
	cli_execute("outfit BootRunaway2");
	bjornify_familiar($familiar[Spooky Pirate Skeleton]);
	cli_execute("autoattack BootRunaway2");
	use_familiar($familiar[Pair of Stomping Boots]);
	cli_execute("equip snow suit");
/* 	if (availableRunaways() >= 5) // For summon hobo underling
		bootsRun(5,place);
	cli_execute("autoattack BootRunaway2");
	cli_execute("outfit BootRunaway2"); */
...
void diner()
{
	...
	// Get some Smithsness just in case
	...
	adventure(3,$location[Sloppy seconds Diner]);
	bjornify_familiar($familiar[Grim Brother]);
	adventure(4,$location[Sloppy seconds Diner]);
	bjornify_familiar($familiar[Golden Monkey]);
...
void farm()
{
	...
	cli_execute("outfit Beach4"); // Last outfit change!
...
void rollover()
{
	...
	use_familiar($familiar[Cheshire Bat]);
